locals {
  ...
  static_webapp_external_ip = "${data.kubernetes_service.static_webapp.metadata[0].name}.${data.kubernetes_service.static_webapp.metadata[0].namespace}.svc.cluster.local:${data.kubernetes_service.static_webapp.spec[0].port[0].port}"
}