...
resource "null_resource" "patch_configmap" {
  triggers = {
    # 리소스를 업데이트하려면 트리거 설정
    configmap_patch = base64sha256(file("${path.module}/deployment_argocd_argocd_repo_server.yaml"))
  }

  provisioner "local-exec" {
    command = <<-EOT
      kubectl apply -f ${path.module}/deployment_argocd_argocd_repo_server.yaml
    EOT
  }
}
